...
#' @export
stackDataFrames3 <- function(NestedFrames, output_format = "tibble", level_names = NULL) {
   ...
   if (is.null(level_names)) {
      ...
      depth_estimate <- function(x) {
         if (is.list(x)) max(sapply(x, depth_estimate), na.rm = TRUE) + 1 else 0
      }
